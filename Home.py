...
st.caption("Models: ANN & Random Forest — trained on the bundled dataset (choose dataset from the sidebar).")
...
# Trigger training on demand and store in session state
...
if submitted:
    if ann_pipe is None or rf_pipe is None:
        st.warning("Models are not trained yet. Click 'Train Models' in the sidebar to train on the selected dataset.")
    else:
        row = pd.DataFrame([{
            "amount": amount,
            "hour": int(hour),
...
st.caption("Trained models are saved in ./models.")
